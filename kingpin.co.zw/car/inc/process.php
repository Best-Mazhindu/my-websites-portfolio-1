...
if(isset($_POST['submit']))  
   {
    ...
    $premium= trim(filter_input(INPUT_POST,"fee",FILTER_SANITIZE_SPECIAL_CHARS));
    //$fee1= trim(filter_input(INPUT_POST,"fee",FILTER_SANITIZE_SPECIAL_CHARS));
    $fee=$premium*4.5/100;
    $add=45.80;
    $total=$fee+$premium+$add;
...
if (empty($name)) 
{
  ...
  (`name`,`surname`,`phone`,`reg_number`,`email`,`address`,`premium`,`fee`,`total`,`buyback`)
 VALUES (:name,:surname,:phone,:reg_number,:email,:address,:premium,:fee,:total,:buyback)"; 
  ...
  { 
     $_SESSION['amount']=$fee;
  echo "<script> alert ('Quotation Successfully Generated'); window.location=('../invoice.php')</script>"; 
} else
